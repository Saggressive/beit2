...
LR=$2
alpha=$3
name=mse_LR${LR}_alpha${alpha}
all_dir=save/distill_beit/${name}
log_dir=save/distill_beit/tensorboard_log/${name}
mkdir -p ${all_dir}
mkdir -p ${log_dir}
nohup /share/miniconda3/envs/beit2/bin/python run_mib_pretraining.py \
    --accum_iter 1 \
    --data_set image_folder \
    --paired_data_path ir_data/flickr_coco_random.json \
    --text_data_path ir_data/text_debug.txt \
    --output_dir ${all_dir} \
    --log_dir ${log_dir} \
