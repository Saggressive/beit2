...
def save_condenser_model_epoch(args, epoch, model, model_without_ddp, optimizer, loss_scaler ,save_ckpt_freq):
    output_dir = Path(args.output_dir)
    epoch_name = str(epoch)

    checkpoint_paths = [output_dir / 'checkpoint']
    opt_paths = [i / 'opt.pth' for i in checkpoint_paths]
    if (epoch + 1) % save_ckpt_freq == 0:
        checkpoint_paths.append(output_dir / ('checkpoint-%s' % epoch_name))
        opt_paths.append(output_dir / ('checkpoint-%s' % epoch_name) / ('opt-%s.pth' % epoch_name))
...
def save_condenser_step(args, model, model_without_ddp, optimizer, loss_scaler, mode="step"):
    output_dir = Path(args.output_dir)
