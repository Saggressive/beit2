...
def train_for_pair(model: torch.nn.Module, condenser_model: torch.nn.Module,condenser_model_without_ddp: torch.nn.Module,
    ...
    txt_input_ids, txt_cl_input_ids,txt_labels, attention_mask = batch["input_ids"],batch["cl_input_ids"], batch["labels"], batch["attention_mask"]

    images = images.to(device, non_blocking=True)
    samples = samples.to(device, non_blocking=True)
    bool_masked_pos = bool_masked_pos.to(device, non_blocking=True)
    txt_input_ids, txt_cl_input_ids,txt_labels, attention_mask = txt_input_ids.to(device, non_blocking=True), txt_cl_input_ids.to(device, non_blocking=True), \
    ...
    with torch.cuda.amp.autocast():  # enabled=False
        ...
        model_input = {"input_ids": txt_input_ids,"cl_input_ids": txt_cl_input_ids, "attention_mask": attention_mask}
...
def train_for_text(condenser_model: torch.nn.Module,condenser_model_without_ddp: torch.nn.Module,
    ...
    with torch.cuda.amp.autocast():  # enabled=False
        ...
        mlm_loss,last_mlm_loss,mid_mlm_loss,cl_loss= condenser_model(model_input, txt_labels, mode="text")

    mlm_loss_value = mlm_loss.item()
    last_mlm_loss_value = last_mlm_loss.item()
    mid_mlm_loss_value = mid_mlm_loss.item()
    cl_loss_value = cl_loss.item()
    if not math.isfinite(mlm_loss_value):
        print(f"Loss is {mlm_loss_value}, stopping training at rank {utils.get_rank()}", force=True)
        sys.exit(1)

    # this attribute is added by timm on one optimizer (adahessian)
    is_second_order = hasattr(optimizer, 'is_second_order') and optimizer.is_second_order
    mlm_loss /=args.accum_iter
    grad_norm = loss_scaler(mlm_loss, optimizer, 
            parameters=condenser_model.parameters(), create_graph=is_second_order,update_grad=(step+1)%args.accum_iter==0)
    if (step+1)%args.accum_iter==0:
        optimizer.zero_grad()
    loss_scale_value = loss_scaler.state_dict()["scale"]
    ...
    if step % print_freq==0:
        logger.info(f" step {complete_step}:  text_last_mlm_loss_value = {last_mlm_loss_value} , \
