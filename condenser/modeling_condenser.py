...
class CondenserForPretraining(nn.Module):
    def __init__(
        ...
        # self.beit_mim_head= nn.ModuleList(
        #     [BertLayer(bert.config) for _ in range(model_args.n_head_layers)]
        # )
        norm_layer = partial(nn.LayerNorm, eps=1e-6)
        self.beit_mim_head = nn.ModuleList([
            Block(
                dim=768, num_heads=12, mlp_ratio=4, qkv_bias=True, norm_layer=norm_layer)
            for i in range(model_args.n_head_layers)])
        
        self.head = nn.Linear(768, 8192)
        self.beit_mlm_head = nn.ModuleList(
            [BertLayer(bert.config) for _ in range(model_args.n_head_layers)]
        )
        self.c_head.apply(self.lm._init_weights)
        # self.beit_mim_head.apply(self.lm._init_weights)
    ...
    def forward(self, model_input, labels, beit_cls=None, beit_hidden=None, mim_labels=None, mim_mask=None,mode="text",args=None):
        ...
        if mode=="text":
            ...
            if self.model_args.late_mlm:
                loss = args.a3*lm_out.loss + args.a0*mlm_loss
                return loss, lm_out.loss , mlm_loss
            else:
                return mlm_loss, torch.tensor(0,dtype=torch.float) , mlm_loss


            
        last_hiddens = lm_out.hidden_states[-1][:,1:]
        beit_mlm_hiddens = torch.cat([beit_cls , last_hiddens], dim=1)
        ...
        loss = args.a0*mlm_loss + args.a1*beit_mlm_loss + args.a2*beit_mim_loss
        if self.model_args.late_mlm:
            loss += args.a3*lm_out.loss

            return loss, beit_mim_loss , beit_mlm_loss , lm_out.loss, mlm_loss
        else:
            return loss, beit_mim_loss , beit_mlm_loss ,torch.tensor(0,dtype=torch.float), mlm_loss
