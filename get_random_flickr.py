...
def get_flickr_caps(split, flickr_entity_dir, output_dir):
    ...
    img_paths = [osp.join("flickr30k-images",id+".jpg") for id in img_ids]
    captions = []
    save_file = []
    ...
    assert len(captions) == len(img_paths)
    for i in range(len(captions)):
        _dict={"caption":captions[i],"image":img_paths[i]}
        save_file.append(_dict)
    # Save captions
    curr_paths_fname = osp.join(output_dir, 'flickr_random_captions.json')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--flickr_entities_dir', type=str, default='/nlp_group/wuxing/suzhenpeng/MCSE/flickr30k_entities')
    parser.add_argument('--flickr_images_dir', type=str, default='/nlp_group/wuxing/suzhenpeng/beit2/ir_data/flickr30k-images')
    parser.add_argument('--output_dir', type=str, default='ir_data/')
    parser.add_argument('--batch_size', type=int, default=32)
    args = parser.parse_args()

    # get random captions
    get_flickr_caps(split='train',flickr_entity_dir=args.flickr_entities_dir,output_dir=args.output_dir)
